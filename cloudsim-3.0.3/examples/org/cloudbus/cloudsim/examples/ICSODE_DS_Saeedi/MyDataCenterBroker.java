...
public class MyDataCenterBroker extends DatacenterBroker implements Parameters {
    ...
    @Override//we have main method but we want to add some things to it-manipulate the main method
    protected void submitCloudlets() {

        Best_runtime = ICSODE_Algorithm();    //farakhani algorithm line 113
        int vmIndex = 0;
    ...
    public double evaluate(List<Double> pos) { //line 396

        double Texe = 0, a;
        int j = 0, i;

        List<Integer> p = new ArrayList<Integer>();
        //convert ashari to adad sahih  //convert peyvasteh to gosasteh
        // vm id ashari bood        
        p = discretemethod(pos); // Find a permutation according to the updated each particle’s position
        // line 413  // For each job Calculate the execution time        
        for (j = 0; j < varsize; j++) {/*25*/
            ...
            // texe = sum of execution times
        }

        Bestsol = p;
        return Texe;  //send to cost of ozve i om jamiat to line 398
    }

    public double ICSODE_Algorithm() {

        varsize = varsize1 / 4;
        System.out.println();
        System.out.println("ICSODE Algorithm Starting....." + "\n");
        Initialization();    //negashte task to vms-tolid pasokh haie avvalieh   line 372

        int it, i, j, k = 0, n, r, anotherRooster, other, ttnum = 0;
        double tempsigma, s1, s2, a;
        double Brt = 0;

        // ********** self-learning coefficient *********** 
        double w = 2, wmax = 0.9, wmin = 0.4, C = 0.4, pCR = 0.2; // Crossover Probability;
        int Mu = 4;
        // ********** self-learning coefficient *********** 

        for (i = 0; i < nPop; i++) {  // 0 to tedade azaie jamiat
        ...
        for (it = 0; it < MaxIt; it++) {
            // main loop
            w = wmin * Math.pow((wmax / wmin), (1 / (1 + 10 * it / MaxIt)));
             // c from nazarieh ashoob
            //************* idea ******************
            C = Mu * C * (1 - Math.pow(C, 2));
            //************* idea ******************

            Timer timer = new Timer();

            MyTimerTask tt = new MyTimerTask(varsize);
            timer.schedule(tt, 500, 2000);// each 500 ms the thread wake up to check
            varsize = tt.event();
            if (varsize > varsize1) {  // task tolid shode va ezafeh shode be varsize bish az astaneh bashtar ast ya na?
                timer.cancel();
                varsize = varsize + 0;
            ...
            for (r = 0; r < rNum; r++) {
                ...
                //************* DE Oprators ***************
                // Mutation
                List<Double> temp_Position_M = new ArrayList<Double>();
                temp_Position_M = Mutation();
                  // baz tarkibe algorith DE
                // Crossover
                List<Double> z = new ArrayList<Double>();
                int j0 = new Random().nextInt(varsize - 1) + 0;
                for (k = 0; k < varsize; k++) {
                    if (k == j0 || new Random().nextDouble() <= pCR) {
                        z.add(temp_Position_M.get(k));
                    } else {
                        z.add(temp_Position.get(k));
                    }

                }

                //************* DE Oprators ***************
                double Cost_DE = evaluate(z);

                // Update Position and Cost
                if (Cost < Cost_DE) {
                    chicken.get(rank.get(r)).setPosition(newtempPosition);
                    chicken.get(rank.get(r)).setcostvalue(Cost);
                } else if (Cost > Cost_DE) {
                    chicken.get(rank.get(r)).setPosition(z);
                    chicken.get(rank.get(r)).setcostvalue(Cost_DE);
            ...
            for (i = rNum + hNum + 1; i < nPop; i++) {
                ...
                temp_Position3 = chicken.get(rank.get(mate.get(i - rNum - hNum))).getPosition();
                ...
                for (j = 0; j < varsize; j++) {
                    ...
                    a = w * temp_Position.get(j) + FL.get(i) * (temp_Position2.get(j) - temp_Position.get(j)) + C * (temp_Position3.get(j) - temp_Position.get(j));
                    a = Math.max(a, varmin);
                    a = Math.min(a, varmax);
    ...
    public void Initialization() {
      // line 116
        int i, j;
        Chickens C1;
        for (i = 0; i < nPop; i++) {   /*number of jamiiat*/
            C1 = new Chickens();

            //Initialization position
            List<Double> position = new ArrayList<Double>();

            for (j = 0; j < varsize; j++) {   /*25*/  //(1/4 of varsize1) make of position
// position of each ozv is a vector from 0 to <varsize
                double a;
            ...
            C1.setPosition(position);   // task to vm

            //Evaluation
            C1.setcostvalue(evaluate(position)); // result recieve from line 110
                // line 88
...
// line 96
        ...
        for (i = 0; i < varsize; i++) {  //25
            // piade sazi formole maghaleh for gosasteh sazi
    ...
    public List<Double> Mutation() {

        List<Double> temp_Position = new ArrayList<Double>();

        List<Double> temp_Pm1 = new ArrayList<Double>();
        List<Double> temp_Pm2 = new ArrayList<Double>();
        List<Double> temp_Pm3 = new ArrayList<Double>();

        List<Integer> B = new ArrayList<Integer>();
// JAHESH
        double beta_min = 0.2;   //Lower Bound of Scaling Factor
        double beta_max = 0.8;   // Upper Bound of Scaling Factor
//        int[] m = new int[3];
        B = randpermB(rNum);
        int m1, m2, m3, i;
        double beta = beta_min + new Random().nextDouble() * (beta_max - beta_min);
        double y;
...
//        }
        temp_Pm1 = chicken.get(rank.get(m1)).getPosition();
        temp_Pm2 = chicken.get(rank.get(m2)).getPosition();
        temp_Pm3 = chicken.get(rank.get(m3)).getPosition();

        temp_Pm1 = convert(temp_Pm1);
        temp_Pm2 = convert(temp_Pm2);
        temp_Pm3 = convert(temp_Pm3);

        for (i = 0; i < varsize; i++) {

            y = temp_Pm1.get(i) + beta * (temp_Pm2.get(i) - temp_Pm3.get(i));
            y = Math.max(y, varmin);
            y = Math.min(y, varmax);
            temp_Position.add(y);
        }
    ...
    public List<Integer> randpermB(int b) {

        int i, r, swap;
        List<Integer> array = new ArrayList<Integer>();
        for (i = 0; i < b; i++) {
            array.add(new Random().nextInt(b - 1) + 0);
        }
...
//    public int[] index(List<Integer> B,int r){
//    
//        int[] m = new int[3];
//        for(int i=0;i<3;i++){
//        m[i]=B.get(i);
